...
|---|---|---|---|---|
| every award's bidder satisfies every required tag; and raising **the qualified bidder's** cost without bound never moves the award to an unqualified one | decision table over (required × reported), plus the perturbation in that direction — or the eligible set observed directly before ranking | `INV-RMF-1` | T3 | table closes over every tag combination · the award never moves. **The direction matters:** sweeping the *unqualified* bidder's cost is flat under a penalty design too, so it passes on the design this row excludes — see [`../verification/control.md §1.1`](../verification/control.md#11-property--decision-table-t3) |
...
|---|---|---|
| CTL-15 | **satisfied in part** — two of its three obligations have a row, and the third is deliberately unowned here. Filter-before-cost is `INV-RMF-1`; bounded award latency and queue depth is `INV-RMF-2`'s bound (the *alerting* half was rejected in [`../invariants/control.md §5`](../invariants/control.md#5-rejection-log) as an operability requirement, not a constraint on the commitment relation); "queue rather than mis-route" is the two of them jointly | §6.1, §6.2, §6.4 |
